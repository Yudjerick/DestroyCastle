...
namespace Dweiss
{
    public static class RigidbodyExtension
    {
               ...
               int stepCount, float timeBeteenStep, Vector3 addedSpeed, Vector3 addedForce)
        {
            ...
            var v = (that.isKinematic == false ? that.linearVelocity : Vector3.zero);// + addedSpeed + addedV;
            var a = (that.useGravity && that.isKinematic == false ? Physics.gravity : Vector3.zero);
            return CalculateMovement(that.transform.position, v, a, stepCount, timeBeteenStep, addedSpeed, addedForce, that.mass, that.linearDamping);
            ...
            Vector3 addedSpeed, Vector3 addedForce)
        {
            ...
            var v = that.linearVelocity + addedSpeed + addedV;
